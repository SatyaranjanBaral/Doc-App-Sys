...
// this functions accept the obj or JSON string
...
// to get doctor by id ie to fetch one doctor
...
export async function updateDoctor(req, res) {
  try {
    ...
    // if exists then update the image else show a 
    if (req.file?.path) {
      const uploaded = await uploadToCloudinary(req.file.path, "doctors");
      if (uploaded) {
...
export async function toggleAvailability(req,res) {
    ...
    const out = normalizeDocForClient (doc.toObject());
    delete out.password;
    return res.json ({success:true,date:out });
