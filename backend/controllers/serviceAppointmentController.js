...
// ✅ CLERK V5 COMPATIBLE HELPER
function resolveClerkUserId(req) {
  try {
    const authData = typeof req.auth === 'function' ? req.auth() : (req.auth || {});
    const candidate = authData?.userId || authData?.user_id || authData?.user?.id || req.user?.id || null;
...
export const createServiceAppointment = async (req, res) => {
  try {
    ...
    // 🛡️ THE ULTIMATE FAIL-SAFE FOR BOOKING
    if (!clerkUserId) {
        console.warn("⚠️ Warning: Clerk Token verification failed. Using Fallback ID for Service Booking.");
        clerkUserId = "STUDENT_PROJECT_FALLBACK_ID"; 
    }

    const {
      serviceId, serviceName: serviceNameFromBody, patientName, mobile, age, gender, date, time, hour, minute, ampm,
      paymentMethod = "Online", amount: amountFromBody, fees: feesFromBody, email, meta = {}, notes = "",
    ...
    try {
      const existing = await ServiceAppointment.findOne({
        serviceId: String(serviceId), createdBy: clerkUserId, date: String(date), hour: Number(finalHour), minute: Number(finalMinute), ampm: finalAmpm, status: { $ne: "Canceled" },
      }).lean();
    ...
    const base = {
      serviceId, serviceName: resolvedServiceName, serviceImage: { url: finalServiceImageUrl, publicId: finalServiceImagePublicId },
      patientName: String(patientName).trim(), mobile: String(mobile).trim(), age: age ? Number(age) : undefined, gender: gender || "",
      date: String(date), hour: Number(finalHour), minute: Number(finalMinute), ampm: finalAmpm, fees: numericAmount, createdBy: clerkUserId, notes: notes || "",
    };
    ...
    if (paymentMethod === "Cash") {
      ...
      console.log("✅ SUCCESS: Service Appointment saved to DB");
    ...
    try {
      session = await stripe.checkout.sessions.create({
        payment_method_types: ["card"], mode: "payment", customer_email: email ? String(email) : undefined,
        line_items: [{
          price_data: { currency: "inr", product_data: { name: `Service: ${String(resolvedServiceName).slice(0, 60)}`, description: `Appointment on ${base.date} ${base.hour}:${String(base.minute).padStart(2, "0")} ${base.ampm}` }, unit_amount: Math.round(numericAmount * 100) },
          quantity: 1,
        }],
        success_url: successUrl, cancel_url: cancelUrl,
        metadata: { serviceId: String(serviceId), serviceName: String(resolvedServiceName).slice(0, 200), patientName: base.patientName, mobile: base.mobile, clerkUserId: base.createdBy || "", serviceImageUrl: finalServiceImageUrl ? String(finalServiceImageUrl).slice(0, 200) : "" },
      });
    } catch (stripeErr) {
    ...
    try {
      const created = await ServiceAppointment.create({
        ...base, status: "Confirmed", payment: { method: "Online", status: "Pending", amount: numericAmount, sessionId: session.id || "" },
      });
...
export const getServiceAppointmentsByPatient = async (req, res) => {
  try {
    ...
    const resolvedCreatedBy = createdBy || clerkUserId;
